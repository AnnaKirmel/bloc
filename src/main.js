...
var isValidHash = (hash) => {
    var firstFourDigits = hash.substring(0, 4);
    var lastThreeDigits = hash.substring(hash.length - 3);

    var isDigitsDivisibleBy3 = firstFourDigits.split('').every(function (digit) {
        return parseInt(digit, 10) % 3 === 0;
    });


    var isLastThreeDigitsSum10 = lastThreeDigits
        .split('')
        .map(function (digit) {
    ...
    return isDigitsDivisibleBy3 && isLastThreeDigitsSum10;
}
var initP2PServer = () => {
...
var handleBlockchainResponse = (message) => {
    ...
    if (latestBlockReceived.index > latestBlockHeld.index) {
        ...
        if (latestBlockHeld.hash === latestBlockReceived.previousHash) {
            ...
            console.log("Received blockchain is longer than current block-chain");
...
var isValidNewBlock = (newBlock, previousBlock) => {
    if (previousBlock.index + 1 !== newBlock.index) {
        ...
        return false;
    } else if (calculateHashForBlock(newBlock) !== new Block.hash) {
        console.log(typeof (newBlock.hash) + ' ' + typeof


            calculateHashForBlock(newBlock));
        console.log('invalid hash: ' + calculateHash - ForBlock(newBlock) + ' ' + newBlock.hash);
        return false;
    }
    return true;
};
var replaceChain = (newBlocks) => {
    if (isValidChain(newBlocks) && newBlocks.length > block - chain.length) {
        console.log('Received blockchain is valid. Replacing cur-rent blockchain with received blockchain');
...
var isValidChain = (blockchainToValidate) => {
    ...
    for (var i = 1; i < blockchainToValidate.length; i++) {
        if (isValidNewBlock(blockchainToValidate[i],
            tempBlocks[i - 1])) {
            tempBlocks.push(blockchainTo - Validate[i]);
        } else {
            return false;
        }
    }
